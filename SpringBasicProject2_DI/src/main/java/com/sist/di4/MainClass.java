...
/*
 *  Spring4 => XML,Annotation => 정보가 노출(보안 취약) => 실무
 *  Spring5 => 자바로 환경 설정(.class) => 보안에 주력 => 차세대 ======> Spring-Boot
 *  
 *  = 모든 클래스를 등록하는 것이 아니다 (기능(컴포넌트)을 가지고 있는 클래스 => 여러 클래스에서 호출)
 *     1) 등록에서 제외 : 데이터형 (자바 기본데이터형, 사용자 정의 데이터형: ~VO)
 *     2) 생성 / 소멸부분은 스프링 자체 담당
 *     3) 특별한 경우가 아니면 싱글턴을 사용한다(한개의 메모리 주소를 이용해서 사용)
 *                       ----- 기술면접 1순위
 *                       ----- DI/AOP
 *                       ----- MVC
 *                       ----- Transaction
