  ...
  +'</table>',
	  methods:{
		  showChef:function(value){
			  eventBus.$emit('showChefEvent',value) //$emit : 이벤트 전송
		  }
	  }
  })
  Vue.component('chef_sub',{
	  template:'<h1>Hello Template2</h1>'
  })
  new Vue({
      el:'.rows',
      data:{
      ...
      updated:function(){
    	  let _this=this;
    	  eventBus.$on('showChefEvent',function(value){  // 위의 전송된 이벤트값을 받는다(이벤트 처리) (위의 명칭과 동일하게 구성)
