	...
	<div class="container">
	  ...
	  <div class="row">
	   <table class="table">
		 ...
		 <tbody>
		  <tr v-for="(vo,index) in movie_list" v-on:click="detail(index)" class="clickTr"> <!-- for문 구현하는 방법 -->
...
<%--
		VO = {} -> Object (JSONObject) =>{"키":값}   ///JSON ->객체표현법
		                                  ---- 멤버변수
...
<script>
  new Vue({
	 ...
	 data:{
		 ...
		 movie_detail:{},
         // JsonObject
         names:'홍길동,이순신,박문수,강감창,김두한,심청이,춘향이,을지문덕'
	 },
	 methods:{
		 detail:function(index){
			 this.movie_detail=this.movie_list[index]  //this를 사용해서 멤버변수를 불러와야한다 
			                                           //this 없이 선언할 경우 지역변수를 불러오는 것
			 $('#dialog').dialog()({
				 autoOpen:false,
				 width:500,
				 height:550,
